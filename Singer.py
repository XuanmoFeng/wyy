...
def get_artists(i,j):
    ...
    csvfile = open('./s.csv', 'w',encoding='utf-8')    # 文件存储的位置
    ...
    for artist in soup.find_all('a', attrs={'class': 'nm nm-icn f-thide s-fc0'}):
        ...
        try:
            sql = "INSERT INTO singer (singerid, name, fenlei,zimu) VALUES ( '%s', '%s','%s','%s' )"
            data=(artist_id,artist_name,i,j)
            cursor.execute(sql % data)
            connect.commit()
            print(artist_id,artist_name,i,j)
